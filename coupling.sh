...
java -jar /usr/local/bin/code-maat-1.0.4-standalone.jar -l /tmp/maat-logfile.log -c git2 > /tmp/maat-org.txt

echo "[" > /var/www/org.json
tail -n +2 /tmp/maat-org.txt | while IFS=, read -r entity n_authors n_revs; do
    echo "{\"entity\": \"$entity\", \"n_authors\": $n_authors, \"n_revs\": $n_revs}," >> /var/www/org.json
done

sed -i '$ s/,$//' /var/www/org.json
echo "]" >> /var/www/org.json
